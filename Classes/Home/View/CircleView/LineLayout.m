...
-(NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (UICollectionViewLayoutAttributes *atts in attsArray) {
        ...
        CustomCollectionViewCell *cell= (CustomCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:atts.indexPath];
        cell.apa = scale;
        if (scale < 0.5) {
            cell.currCircle = NO;
        }else{
...
-(CGPoint)targetContentOffsetForProposedContentOffset:(CGPoint)proposedContentOffset withScrollingVelocity:(CGPoint)velocity {
    ...
    }else if (proposedContentOffset.x <= 160 *Main_Screen_Width / 375.0 ) {
        proposedContentOffset.x = 160 * Main_Screen_Width / 375.0 - 20;
    }else{
        proposedContentOffset.x = (160 * Main_Screen_Width / 375.0 - 20) * 2 - 0.1;
    }
