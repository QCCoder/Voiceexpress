...
- (void)reloadData
{
    if (_dataSource)
    {
        ...
        NSUInteger sliceCount = [_dataSource numberOfSlicesInCircleChart:self];
        
        double sum = 0.0;
        double values[sliceCount];
        for (int index = 0; index < sliceCount; index++) {
            values[index] = [_dataSource circleChart:self valueForSliceAtIndex:index];
            sum += values[index];
        }
        ...
        for(int index = 0; index < sliceCount; index ++)
        {
            ...
            UIColor *color = nil;
            if([_dataSource respondsToSelector:@selector(circleChart:colorForSliceAtIndex:)])
            {
                color = [_dataSource circleChart:self colorForSliceAtIndex:index];
            }
            ...
            if([_dataSource respondsToSelector:@selector(circleChart:textForSliceAtIndex:)])
            {
                layer.text = [_dataSource circleChart:self textForSliceAtIndex:index];
            }
...
- (void)notifyDelegateOfSelectionChangeFrom:(NSUInteger)previousSelection to:(NSUInteger)newSelection
{
    if (previousSelection != newSelection){
        if(previousSelection != -1){
            ...
            if ([_delegate respondsToSelector:@selector(circleChart:willDeselectSliceAtIndex:)])
                [_delegate circleChart:self willDeselectSliceAtIndex:tempPre];
            [self setSliceDeselectedAtIndex:tempPre];
            previousSelection = newSelection;
            if([_delegate respondsToSelector:@selector(circleChart:didDeselectSliceAtIndex:)])
                [_delegate circleChart:self didDeselectSliceAtIndex:tempPre];
        }
        
        if (newSelection != -1){
            if([_delegate respondsToSelector:@selector(circleChart:willSelectSliceAtIndex:)])
                [_delegate circleChart:self willSelectSliceAtIndex:newSelection];
            [self setSliceSelectedAtIndex:newSelection];
            _selectedSliceIndex = newSelection;
            if([_delegate respondsToSelector:@selector(circleChart:didSelectSliceAtIndex:)])
                [_delegate circleChart:self didSelectSliceAtIndex:newSelection];
        }
        ...
        if(_selectedSliceOffsetRadius > 0 && layer){
            if (layer.isSelected) {
                if ([_delegate respondsToSelector:@selector(circleChart:willDeselectSliceAtIndex:)])
                    [_delegate circleChart:self willDeselectSliceAtIndex:newSelection];
                [self setSliceDeselectedAtIndex:newSelection];
                if (newSelection != -1 && [_delegate respondsToSelector:@selector(circleChart:didDeselectSliceAtIndex:)])
                    [_delegate circleChart:self didDeselectSliceAtIndex:newSelection];
                ...
                if (newSelection != -1 && [_delegate respondsToSelector:@selector(circleChart:didSelectSliceAtIndex:)])
                    [_delegate circleChart:self didSelectSliceAtIndex:newSelection];
            }
        }
    }
