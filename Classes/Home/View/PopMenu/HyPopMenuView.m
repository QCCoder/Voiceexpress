...
#define CancelStrImgaeName @"icon-arr-t"
...
-(instancetype) initWithItmes:(NSArray<MenuLabel *> *)Itmes
{
    ...
    if (self) {
        _is = true;
        _ItmesArr = Itmes;
        [self setFrame:CGRectMake(0, 64, kW, kH - 64)];
        [self initUI];
        [self show];
...
-(void)initUI
{
    ...
    if (version >= 8.0f) {
        ...
        [BlurView setBackgroundColor:[UIColor colorWithRed:1 green:1 blue:1 alpha:1.0f]];
    }
    ...
    CGFloat DownY = kH - 40 - 64;
    [DownView setTag:10];
    [DownView setFrame:(CGRect){{0,DownY},{kW,40}}];
    
    CGFloat CANCELw = 11;
    ImageView *CancelButton = [[ImageView alloc] initWithFrame:CGRectMake(CGRectGetWidth(DownView.bounds)/2 - CANCELw/2, CGRectGetHeight(DownView.bounds)/2 - CANCELw/2, 11, 13)];
...
-(void)CirculatingItmes
{
    ...
    for (MenuLabel *Obj in _ItmesArr) {
        ...
        buttonY = ((index / HowMucHline) * (ButtonHigh +10)) + (kH * 0.04);
        CGRect fromValue = CGRectMake(([UIScreen mainScreen].bounds.size.width -ButtonWidth) * 0.5, CGRectGetMinY(downView.frame), ButtonWidth, ButtonHigh);
...
//            [_BlurView addGestureRecognizer:tap];
            ...
            [_BlurView addGestureRecognizer:swip];
        } HideDisplay:false];
        index ++;
    }
}
...
-(void)DismissCompletionBlock:(void(^) (BOOL CompletionBlock)) completionBlock{
    ...
    for (MenuLabel *label in _ItmesArr) {
        ...
        CGRect toValue = CGRectMake(([UIScreen mainScreen].bounds.size.width - ButtonWidth ) *0.5, kH, ButtonWidth, ButtonHigh);
