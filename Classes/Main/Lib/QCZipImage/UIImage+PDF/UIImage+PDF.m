...
+(UIImage *)imageHalfPDFWithContentsOfFile:(NSString *)path{
    if([[ path pathExtension ] isEqualToString: @"pdf" ])
    {
        return [ UIImage halfSizeImageWithPDFURL:[ NSURL fileURLWithPath:path ]];
    }
    else
    {
        return [ UIImage imageWithContentsOfFile:path ];
...
+(UIImage *)halfSizeImageWithPDFURL:(NSURL *)URL{
    return [UIImage halfSizeImageWithPDFURL:URL atPage:1];
}

+(UIImage *)halfSizeImageWithPDFURL:(NSURL *)URL atPage:(NSUInteger)page{
    if ( URL == nil )
        return nil;
    
    CGRect mediaRect = [ PDFView mediaRectForURL:URL atPage:page];
    
    return [ UIImage imageWithPDFURL:URL atSize:CGSizeMake(mediaRect.size.width * 0.5, mediaRect.size.height * 0.5) atPage:page preserveAspectRatio:YES ];
}
