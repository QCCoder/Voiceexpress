...
-(void)showcontentWithModelString:(NSString *)content{
    ...
    NSString *pathTemplate = [[NSBundle mainBundle] pathForResource:@"SNContentTemplate" ofType:@"html"];
    NSString *textTemplate = [NSString stringWithContentsOfFile:pathTemplate encoding:NSUTF8StringEncoding error:nil];
    textTemplate = [textTemplate stringByReplacingOccurrencesOfString:@"{CONTENT}" withString:content];
    textTemplate = [textTemplate stringByReplacingOccurrencesOfString:@"{FONTSIZE}" withString:[NSString stringWithFormat:@"%ld",(long)[VEUtility contentFontSize]]];
    self.currentContent = [textTemplate stringByReplacingOccurrencesOfString:@"{LINEHEIGHT}" withString:[NSString stringWithFormat:@"%lf",(long)[VEUtility contentFontSize] * 1.7]];
    
    [self showContentWithString:self.currentContent];
}

-(void)reload{
    [self showcontentWithModelString:_content];
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self.viewContent setFrame:CGRectMake(0,0, frame.size.width - self.scrollbarWidth, frame.size.height)];
        ...
        [self.viewWeb setUserInteractionEnabled:YES];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    float h;
    NSString *str =[webView stringByEvaluatingJavaScriptFromString:@"document.getElementById(\"textContent\").offsetHeight"];
    CGFloat documentHeight = [str floatValue];
    h = documentHeight + 20;
    self.viewWeb.frame = CGRectMake(self.viewWeb.frame.origin.x, self.viewWeb.frame.origin.y, self.viewWeb.frame.size.width, h);
    self.contentHeight = self.viewWeb.frame.origin.y + h;
    [self.viewContent setContentSize:CGSizeMake(self.viewWeb.frame.size.width, self.contentHeight)];
    
    [self initScroll];
    
    [webView stopLoading];
    if ([self.delegate respondsToSelector:@selector(didFinishLoadWebView:contentHight:)]) {
        [self.delegate didFinishLoadWebView:webView contentHight:self.contentHeight];
    }
}
