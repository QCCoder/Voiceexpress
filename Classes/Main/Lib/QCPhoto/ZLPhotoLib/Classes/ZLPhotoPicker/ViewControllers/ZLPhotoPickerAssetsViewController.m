...
static CGFloat CELL_MARGIN = 3;
static CGFloat CELL_LINE_MARGIN = 3;
...
- (ZLPhotoPickerCollectionView *)collectionView{
    if (!_collectionView) {
        ...
        CGFloat cellW = (self.view.frame.size.width -10 - CELL_MARGIN * CELL_ROW + 1) / CELL_ROW;
        ...
        collectionView.contentInset = UIEdgeInsetsMake(kMarginItemWH, 0,TOOLBAR_HEIGHT, 0);
        collectionView.collectionViewDelegate = self;
        [self.view insertSubview:_collectionView = collectionView belowSubview:self.toolBar];
        
        NSDictionary *views = NSDictionaryOfVariableBindings(collectionView);
        
        NSString *widthVfl = @"H:|-5-[collectionView]-5-|";
...
- (void) setupButtons{
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem itemWithTarget:self action:@selector(backPop) image:@"tab_icon_back" highImage:nil];
}
