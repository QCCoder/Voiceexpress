...
/**
 *  imageView展示网络图片,刷新缓存方式
 *
 *  @param urlStr  图片地址
 *  @param phImage 占位图片
 */
-(void)imageWithRefreshUrlStr:(NSString *)urlStr phImage:(UIImage *)phImage{
    
    if(urlStr==nil) return;
    
    NSURL *url=[NSURL URLWithString:urlStr];
    
    [self sd_setImageWithURL:url  forState:UIControlStateNormal placeholderImage:phImage options:SDWebImageRetryFailed | SDWebImageRefreshCached];
}
