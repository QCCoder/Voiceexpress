...
+(void)show:(UIViewController *)handleVC type:(PhotoBroswerVCType)type index:(NSUInteger)index photoModelBlock:(NSMutableArray *(^)())photoModelBlock{
...
//    [itemModel.sourceImageView removeFromSuperview];
//    [self.photoModels removeObject:itemModel];
//    self.pageCount--;
//    if (self.photoModels.count >= 3) {
...
//    [self.visiblePhotoItemViewDictM removeObserver:itemView forKeyPath:NSStringTo(@"%d",self.page + 1)];
...
//    [[NSNotificationCenter defaultCenter]postNotificationName:@"removeImage" object:nil userInfo:nil];
