...
-(void)layoutSubviews{
    ...
    for (int i = 0; i < self.nodeArray.count; ++i) {
        ...
        CGFloat width = 70 / 375.0 * Main_Screen_Width;
        CGFloat margin = (min -  width * 3)/2;
        CGFloat height = width;
        int row = i % 3;
        int column = i / 3;
        CGRect frame = CGRectMake(row *(margin + width), column * (margin + height), width, height);
...
//    if(_viewState != viewState){
        ...
        switch (_viewState){
            ...
            case YLSwipeLockViewStateSelected:
                [self performSelector:@selector(cleanNodes) withObject:nil afterDelay:1];
                break;
            default:
                break;
        }
